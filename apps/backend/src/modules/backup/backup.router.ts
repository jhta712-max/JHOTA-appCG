...
async function generateBackup() {
  ...
  const [
    ...
    projectAnticipos, projectItems, projectCategoryBudgets, projectSuppliers,
    ...
    whatsappConversations, whatsappMessages, whatsappAuditLogs,
    ...
    safe(() => prisma.projectSupplier.findMany()),
    ...
    safe(() => prisma.whatsAppAuditLog.findMany({ take: 1000, orderBy: { createdAt: 'desc' } })),
    safe(() => prisma.projectExtraordinaryExpense.findMany()),
    safe(() => prisma.ocrJob.findMany({ orderBy: { createdAt: 'desc' } })),
