...
export async function forgotPassword(email: string) {
  ...
  if (!user) {
    throw new AppError(401, 'Si el correo existe, recibirás instrucciones de reset', 'USER_NOT_FOUND');
  }
  ...
  return {
    message: 'Si el correo existe en nuestro sistema, recibirás instrucciones para resetear tu contraseña',
...
export async function resetPassword(token: string, newPassword: string) {
  ...
  if (!reset) {
    throw new AppError(400, 'Token de reset inválido', 'INVALID_TOKEN');
  }

  if (reset.usedAt) {
    throw new AppError(400, 'Este token ya fue utilizado', 'TOKEN_USED');
  }

  if (reset.expiresAt < new Date()) {
    throw new AppError(400, 'El token ha expirado. Solicita uno nuevo', 'TOKEN_EXPIRED');
  }
