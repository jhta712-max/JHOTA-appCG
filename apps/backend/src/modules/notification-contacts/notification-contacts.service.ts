...
export async function createContact(data: { name: string; phone?: string | null; email?: string | null }, userId: string) {
  return prisma.notificationContact.create({
    data: { name: data.name, phone: data.phone ?? null, email: data.email ?? null, createdById: userId },
    select: SELECT,
  });
}

export async function updateContact(id: string, data: { name?: string; phone?: string | null; email?: string | null; isActive?: boolean }) {
